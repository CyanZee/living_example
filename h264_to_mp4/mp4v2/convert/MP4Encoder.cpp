...
int MP4Encoder::WriteH264Data(MP4FileHandle hMp4File,const unsigned char* pData,int size)
{
	...
	////printf("pData[0] : %d, pData[1] : %d, pData[2] : %d, pData[3] : %d\n", pData[0], pData[1], pData[2], pData[3]);
	while (len = ReadOneNaluFromBuf(pData,size,pos,nalu))
	{
		////printf("00000000000 len : %d\n", len);
#if 1
		 if(nalu.type != 7 && nalu.type != 8 && nalu.type != 5 && nalu.type != 1 && nalu.type != 6)
	  	 {
	  	 	//flag = 1;
			printf("&&&&&&&&&&&&&& nal_type : %x \n", nalu.type);
			pos += len;
		...
		if(nalu.type == 0x07 && sps_str == 0) // sps
		{
			sps_str = 1;
			// 添加h264 track    
			m_videoId = MP4AddH264VideoTrack
	...
	while(i<nBufferSize)
	{
			...
			)
		{
			...
			while (pos<nBufferSize)
			{
				...
				if(readNULL((char *)&buffer[pos]))
				{
					//int nul_type = buffer[pos] & 0x1f;
					//if(nul_type == 1 || nul_type == 5 || nul_type == 7 || buffer[pos] == 0x68 || nul_type == 6)
					break;
				}
				else
				{
...
bool MP4Encoder::WriteH264File(const char* pFile264,const char* pFileMp4)
{
	...
	MP4FileHandle hMp4File = CreateMP4File(pFileMp4,704,576);
	...
	while(1)
	{
		...
		printf("2222222222222 readlen : %d \n", readlen);

		printf("2222222222222 writelen : %d \n", writelen);
		writelen = WriteH264Data(hMp4File,buffer,writelen);
		printf("2222222222222 writelen end : %d \n", writelen);
		if(writelen<=0)
		{
			break;
		}
